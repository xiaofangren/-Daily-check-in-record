...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const acctDbOk = await initAcctDB();
  if (!acctDbOk) {
    console.warn('记账数据库初始化失败');
  }
...
function initUI() {
  ...
  document.getElementById('btn-export-acct').addEventListener('click', exportAccountingCSV);
  document.getElementById('btn-import-acct').addEventListener('click', () => {
    document.getElementById('import-file-acct').click();
  });
  document.getElementById('import-file-acct').addEventListener('change', importAccountingCSV);
...
function switchView(viewId) {
  ...
  const acctFab = document.getElementById('acct-fab');
  if (acctFab) acctFab.style.display = viewId === 'accounting-view' ? 'flex' : 'none';
  ...
  if (viewId === 'accounting-view') {
    if (typeof initAccounting === 'function' && !window._acctInited) {
      initAccounting();
      window._acctInited = true;
    } else if (typeof renderAcctMonth === 'function') {
      renderAcctMonth();
    }
  }
...
async function confirmClearData() {
  showConfirm('确定清除全部数据？此操作不可恢复！', async () => {
    try {
      await clearAllData();
      if (typeof clearAllBills === 'function') {
        await clearAllBills();
      }
      showToast('数据已清除');
      renderCheckinView();
      if (typeof renderAcctMonth === 'function') {
        renderAcctMonth();
      }
    } catch (e) {
